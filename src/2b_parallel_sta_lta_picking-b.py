...
outdir = f'{home}/event_detection/sta_lta/sta_lta_mpi'
...
for index in recvbuf:
    ...
    for stn_type in stn_types:
        ...
        for i_file in range(len(bb_files_Z)):
            ...
            true_p = UTCDateTime(true_arriv)
            sig_ind = np.where(st[0].times('UTCDateTime') >= true_p-0.1)[0][0]
            ...
            if len(thresh_sig_Z)>0:
                ...
                obspy_res.append(UTCDateTime(true_arriv) - pick)
            else:
            ...
            # thresh_sig_N = thresh_ind_N[np.where(thresh_ind_N >= sig_ind)[0]]
            # thresh_sig_E = thresh_ind_E[np.where(thresh_ind_E >= sig_ind)[0]]
            # thresh_sig_Z = thresh_ind_Z[np.where(thresh_ind_Z >= sig_ind)[0]]
                                    
            
            # # Check for each component if algorithm was triggered
            # if len(thresh_sig_N)>0:
            #     trig_ind = np.append(trig_ind,0)
            #     obspy_pick_times = np.append(obspy_pick_times,st_vel_filt_N[0].times('UTCDateTime')[thresh_sig_N[0]])
            # if len(thresh_sig_E)>0:
            #     trig_ind = np.append(trig_ind,1)
            #     obspy_pick_times = np.append(obspy_pick_times,st_vel_filt_E[0].times('UTCDateTime')[thresh_sig_E[0]])
            # if len(thresh_sig_Z)>0:
            #     trig_ind = np.append(trig_ind,2)
            #     obspy_pick_times = np.append(obspy_pick_times,st_vel_filt_Z[0].times('UTCDateTime')[thresh_sig_Z[0]])
            
            # # If triggered, add pick time and residuals to lists
            # if len(trig_ind) > 0:
            #     pick_ind = np.where(obspy_pick_times == np.min(obspy_pick_times))[0][0]
            #     pick = np.min(obspy_pick_times)
            #     obspy_triggered.append('Yes')
            #     obspy_pick.append(pick)
            #     obspy_res.append(UTCDateTime(true_arriv) - pick)
            ...
            #         folder = 'negative'
            #     fig, axs = plt.subplots(2,1)
            #     axs[0].plot(st_vel_filt_E[0].times(),st_vel_filt_E[0].data,color='k')
            #     if len(np.where(cft_E>thresh)[0]>0):
            #         p_ind = np.where(cft_E>thresh)[0][0]
            #         axs[0].vlines(st_vel_filt_E[0].times()[p_ind],np.min(st_vel_filt_E[0].data),np.max(st_vel_filt_E[0].data),color='red',label='P-arrival pick',lw=1)
            #     axs[0].grid(linestyle='-',alpha=0.5)
            #     axs[0].vlines(UTCDateTime(true_arriv)-UTCDateTime(st_vel_filt_E[0].stats.starttime),np.min(st_vel_filt_E[0].data),np.max(st_vel_filt_E[0].data),color='blue',label='True P-arrival',lw=1)
            #     axs[0].set_xlabel('Time (s)')
            #     axs[0].set_ylabel('Amplitude (m/s)')
            #     axs[0].legend(facecolor='white')
            #     axs[0].text(0.98,0.1,f'M{round(mag,1)}',ha='right',transform = axs[0].transAxes)
            ...
            #     axs[1].plot(st_vel_filt_E[0].times(),cft_E,color='k')
            #     axs[1].grid(linestyle='-',alpha=0.5)
            #     axs[1].hlines(thresh, np.min(st_vel_filt_E[0].times()), np.max(st_vel_filt_E[0].times()),
            #                   ls='--', lw=1, color='red',label='triggering threshold')
            #     axs[1].set_xlabel('Time (s)')
            #     axs[1].set_ylabel('STA/LTA')
            #     axs[1].legend(facecolor='white')
            ...
            #     plt.savefig(f'{figdir}/{folder}/{run}_{stn}.E_{sta}-{lta}-{thresh}_vel.png',dpi=300)
            #     plt.close()
...
data = {'Batch':batches,'Run':rupts,'Magnitude':mags,'Station':stns,
        '0.05/5/20_obspy_triggered':obspy_triggered,
        '0.05/5/20_obspy_P-pick time':obspy_pick,'0.05/5/20_obspy_P-arrival Residual (s)':obspy_res}
