...
gold_dfs = sorted(glob(f'{home}/ONC-EEW/magnitude_estimation/GM20_pd-Mw_scaling/*.csv'))

# Read in file with observed Pd values (from Trugman et al., 2019)
DTAmps=genfromtxt(f'{home}/ONC-EEW/files/DT2019_files/DT2019_Amplitudes.txt')
...
for i in range(len(windows)):
    ...
    ax.scatter(obs_M,obs_Pd[:,i],marker='d',c='indigo',ec='indigo',lw=0.1,s=15,alpha=0.6, label=r'$\mu_{E,obs}$ (Trugman et al., 2019)')
    ...
    for j, dist_lim in enumerate(dist_lims):
        ...
        ax.scatter(mw,Pd,marker='+',c=colors[j],ec=colors[j],lw=1,s=15,alpha=0.6,label=r'$\mu_{E,sim}$ '+f'< {dist_lim}km')
...
for h in handles:
    if isinstance(h, mpatches.Patch):
        # Already a patch — just force opaque
        proxy = mpatches.Patch(color=h.get_facecolor(), label=h.get_label(), alpha=1)
    elif hasattr(h, 'get_facecolors'):  # For PathCollection (scatter)
        fc = h.get_facecolors()[0] if len(h.get_facecolors()) > 0 else 'gray'
        ec = h.get_edgecolors()[0] if len(h.get_edgecolors()) > 0 else 'black'
        proxy = mpatches.Circle((0,0), radius=5,
                                facecolor=fc,
                                edgecolor=ec,
                                label=h.get_label(),
                                linewidth=0.5,
...
axs[1,1].legend(opaque_handles,labels,bbox_to_anchor=(0.5,-0.25),loc='upper center',facecolor='white',fontsize=10,framealpha=0.98,ncol=3,markerscale=1.5)
...
plt.savefig(f'{home}/ONC-EEW/manuscript/figures/Fig4_Pd-MW_dist-split_v2.png',dpi=300)
